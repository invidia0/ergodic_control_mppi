...
\end{equation}
Here, $q_{t,i}^{\mathrm{rec}}$ is the normalized recency weight assigned to
the spatial point $\mathbf m_{t,i}$. Since
$q_{t,i}^{\mathrm{rec}}\geq0$ and $\sum_i q_{t,i}^{\mathrm{rec}}=1$, these
weights define a probability mass function over the memory points.
...
Figure~\ref{fig:relative_excess} illustrates this comparison at one selected
memory point and displays the corresponding value of~\eqref{eq:relative_excess}.

\begin{figure*}[t]
    \color{black}
    \centering
    \includegraphics[width=\textwidth]{pictures/fig_excess_focus.pdf}
    \caption{Pointwise construction of the relative over-coverage at the coarse
    scale. (a) Fading occupancy over the workspace, with the selected
    neighborhood boxed. (b) Magnified neighborhood: grayscale shows
    $o_t^{h_c}$, blue contours show the scale-matched target $p_{h_c}^\star$,
    the circle has radius $\sqrt{h_c/2}$, and the solid red line selects the
    spatial slice through $\mathbf m_{t,i^\star}$. (c) Occupancy and target along
    that slice; the shaded region is $[o_t^{h_c}-p_{h_c}^\star]_+$, and the inset
    evaluates~\eqref{eq:relative_excess} at the selected memory point.}
    \label{fig:relative_excess}
\end{figure*}
