...
\begin{itemize}[leftmargin=*]
    ...
    \item \cl{We drive long-run coverage with a constant-size fading-memory feedback whose repulsion is weighted by the online occupancy deficit, an in-closed-form surrogate for the ergodic-metric gradient.}
...
\end{equation}}
\cl{parameterized by a single lengthscale $\ell>0$. We keep $\ell$ as an explicit subscript because the reference field ultimately combines this kernel at several lengthscales: the attraction term below sets $\ell$ adaptively, and the memory feedback of Sec.~\ref{sec:memory_flow} reuses the kernel at a coarse and a fine scale.}
...
The reference flow of Sec.~\ref{sec:target_flow}, built from the in-horizon
ensemble $\hat\pi_t$, spreads the sampled rollouts within a single planning
horizon but carries no representation of where the robot has already spent
time. On its own it does not induce long-run ergodic coverage: the score term
attracts the executed trajectory toward the nearest mode, and the in-horizon
repulsion cannot prevent it from settling there. We therefore augment the
repulsion with feedback from a bounded, fading memory of executed visitation.

Let $\mathcal M_t=(\mathbf m_{t,1},\dots,\mathbf m_{t,P})$ be a fixed-size
buffer holding the last $P$ executed spatial positions, oldest first, with
$\mathbf m_{t,i}=\Pi(\mathbf x_{t-P+i})$. It is maintained by a deterministic
shift,
\begin{equation}\label{eq:memory_shift}
\mathcal M_{t+1}
=
\big(\mathbf m_{t,2},\dots,\mathbf m_{t,P},\,\Pi(\mathbf x_{t+1})\big).
\end{equation}
With geometric recency weights $\omega_i=\gamma^{\,P-i}$, $\gamma\in(0,1)$
(so the newest entry has weight $1$ and older ones fade with time constant
$\tau\approx 1/(1-\gamma)$), the fading memory defines the empirical measure
\begin{equation}\label{eq:fading_measure}
\hat\pi^{\mathcal M}_t(\mathbf z)
=
\frac{1}{\sum_{i}\omega_i}\sum_{i=1}^{P}\omega_i\,
\delta\!\big(\mathbf z-\mathbf m_{t,i}\big),
\end{equation}
and, through a kernel of bandwidth $h$, the \emph{fading occupancy density}
\begin{equation}\label{eq:occupancy_density}
o^{h}_t(\mathbf z)
=
\frac{\sum_{i=1}^{P}\omega_i\,\kappa_h(\mathbf m_{t,i},\mathbf z)}
{\big(\sum_i\omega_i\big)\,\eta_h},
\end{equation}
where $\eta_h=\int_{\Omega}\kappa_h(\mathbf 0,\mathbf y)\,d\mathbf y$ normalizes
$o^{h}_t$ to integrate to approximately one. This is an online kernel estimate
of the very visitation law that Sec.~\ref{sec:guarantees} characterizes
asymptotically.

Coverage is driven by repelling the plan from over-covered regions only, at two
complementary spatial scales. The generic building block is the
\emph{weighted memory-repulsion operator at bandwidth $h$}, evaluated on a
vector of non-negative per-particle weights
$\mathbf w=(w_1,\dots,w_P)\in\mathbb R_{\ge0}^{P}$ (one per memory point),
\begin{equation}\label{eq:memory_repulsion}
\boldsymbol\rho^{h}_t(\mathbf z;\mathbf w)
=
\frac{\sum_{i=1}^{P} w_{i}\,
\mathbf C\,\nabla_{\mathbf x}\kappa_h(\mathbf x,\mathbf z)\big|_{\mathbf x=\mathbf m_{t,i}}}
{\sum_{i=1}^{P} w_{i}},
\end{equation}
a $\mathbf w$-weighted average of curl-preconditioned pushes away from the
memory points, the entry $w_i$ being the scalar weight of point
$\mathbf m_{t,i}$. The two scales below instantiate $\mathbf w$ with the
coverage-deficit weight vectors
$\mathbf w^{c}=(w^{c}_{t,i})_{i=1}^{P}$~\eqref{eq:coarse_weight} and
$\mathbf w^{f}=(w^{f}_{t,i})_{i=1}^{P}$~\eqref{eq:fine_weight}, whose entries are
defined below. Each gradient
$\nabla_{\mathbf x}\kappa_h(\mathbf x,\mathbf z)|_{\mathbf x=\mathbf m}
=\tfrac{2}{h}(\mathbf z-\mathbf m)\,\kappa_h(\mathbf m,\mathbf z)$ points from
$\mathbf m$ toward $\mathbf z$, i.e.\ repels $\mathbf z$ from $\mathbf m$,
strongest when the two are close.
Normalizing by $\sum_i w_i$ makes the operator invariant to the overall scale of
the weights, so only their relative pattern acts and the field vanishes
gracefully as $\sum_i w_i\to0$. We apply the same constant preconditioner
$\mathbf C=\mathbf R(\theta)$ that shapes the attraction~\eqref{eq:stein_operator_A}:
since $\mathbf C$ rotates each radial push into a tangential one, the repulsion
makes the robot \emph{circle} an over-covered region, laying down the next
concentric orbit, rather than fleeing it radially; applying the same $\mathbf C$
to attraction and repulsion also keeps the composite field on a single,
consistent curl geometry, and, being non-singular, preserves the equilibrium
structure (Remark~\ref{rem:preconditioning}).

\paragraph{Coarse scale: inter-mode distribution}
At a coarse (mode-scale) bandwidth $h_c$ we form the weight
\begin{equation}\label{eq:coarse_weight}
\begin{aligned}
w^{c}_{t,i}
={}&
(1-\lambda_d)\,\omega_i\\
&+
\lambda_d\,\big[\,o^{h_c}_t(\mathbf m_{t,i})-p^\star(\mathbf m_{t,i})\,\big]_{+}\,g_{t,i},
\end{aligned}
\end{equation}
which blends, through the gate $\lambda_d\in[0,1]$, pure recency ($\lambda_d=0$) with a
coverage deficit ($\lambda_d=1$). With $\lambda_d=1$ the weight is nonzero only
where a whole mode already holds more than its target share of recent occupancy,
so the induced push ejects the robot from filled modes and, being silent on
under-covered ones, distributes dwell time toward $p^\star$. The binary
\emph{fill gate}
\begin{equation}\label{eq:fill_gate}
g_{t,i}
=
\mathbf 1\!\big[\,o^{h_f}_t(\mathbf m_{t,i})\ge p^\star(\mathbf m_{t,i})\,\big]
\end{equation}
switches the ejection off until the region is also filled at the fine scale
$h_f$ (next paragraph), decoupling ejection \emph{strength} from ejection
\emph{timing}: a strong ejection can be held dormant while a mode is still being
filled and fire only once the fill is dense.

\paragraph{Fine scale: within-mode fill}
At a fine (strip-scale) bandwidth $h_f\ll h_c$ we form the weight
\begin{equation}\label{eq:fine_weight}
\begin{aligned}
w^{f}_{t,i}
={}&
(1-\lambda_s)\,\omega_i\\
&+
\lambda_s\,\big[\,o^{h_f}_t(\mathbf m_{t,i})-p^\star(\mathbf m_{t,i})\,\big]_{+},
\end{aligned}
\end{equation}
which blends, through $\lambda_s\in[0,1]$, the fading immediate trail ($\lambda_s=0$)
with a fine-scale deficit ($\lambda_s=1$). With $\lambda_s=0$ the most recent
orbit repels the next one outward by a small tangential (curl-rotated) step,
producing the dense concentric spiral that fills a mode.

\paragraph{The two-scale game}
The coarse and fine terms answer complementary questions and pull on the same
trajectory in opposition. The coarse term asks ``has this \emph{mode} received
its target share?'' and, once it has, drives the robot out toward another mode;
used alone, a mode-scale bandwidth declares a mode ``covered'' after only a
couple of orbits and ejects before the interior is filled. The fine term asks
``is this narrow \emph{strip} already visited?'' and steps each orbit onto the
next unfilled strip, densely tiling the mode; used alone, it has no notion of a
mode's global share and over-dwells. Running both, tied together by the fill
gate~\eqref{eq:fill_gate}, produces the target behaviour: \emph{dwell and
densely fill a mode at the fine scale, and only once it is filled let the coarse
scale eject and carry the robot to the next mode}. The scale separation
$h_f\ll h_c$ is precisely what lets the two objectives, dense local fill and
proportional global distribution, be expressed at once: the scale that decides
``mode full'' is far coarser than the scale that decides ``strip filled,'' so a
single bandwidth cannot serve both. In the present work the toggles are fixed at
$(\lambda_d,\lambda_s)=(1,0)$ with the gate enabled, the configuration found to
jointly maximize fill density and mode coverage; we retain them as explicit
parameters because they cleanly separate the two mechanisms and are natural
ablation axes.

\paragraph{Assembled reference field}
Collecting the attraction~\eqref{eq:stein_reference_flow_A} and the two
memory-repulsion terms, and optionally normalizing to a fixed reference speed,
the field actually tracked is
\begin{equation}\label{eq:augmented_reference_flow}
\begin{aligned}
\widetilde{\mathbf h}^{\mathcal M}_t(\mathbf z)
=
\Pi_{v}\Big[
\widetilde{\mathbf h}_t(\mathbf z)
&+
r_w\,\boldsymbol\rho^{h_c}_t(\mathbf z;\mathbf w^{c})\\
&+
s_w\,\boldsymbol\rho^{h_f}_t(\mathbf z;\mathbf w^{f})
\Big],
\end{aligned}
\end{equation}
with coarse and fine gains $r_w,s_w\ge 0$ and the speed gauge
\begin{equation}\label{eq:speed_gauge}
\Pi_{v}[\mathbf u]
=
\begin{cases}
v\,\mathbf u/\max(\|\mathbf u\|,\varepsilon), & v>0,\\[2pt]
\mathbf u, & v=0,
\end{cases}
\end{equation}
which, for $v>0$, tracks the flow \emph{direction} at constant speed $v$
(matching the constant-speed reference of the LQR flow-matching baseline) and,
for $v=0$, keeps the raw velocity magnitude. The occupancy-awareness carries
over from the single-scale case: where a region is under-covered the deficit
weights vanish and the score attraction is free to dwell, while over-covered
regions repel in proportion to their excess. Since~\eqref{eq:ergodic_metric}
penalizes $[\,o(\cdot)-p^\star(\cdot)\,]^2$ across scales, $-\nabla$ of that
discrepancy points down exactly the positive-deficit regions, so each
memory-repulsion term is a local, closed-form surrogate for the ergodic-metric
gradient at its scale, realized from $\mathcal M_t$ without an online PDE solve.
When $v>0$ the normalization discards $\|\widetilde{\mathbf h}^{\mathcal M}_t\|$,
so the attraction (implicit unit gain) fixes the overall scale and $r_w,s_w$ act
as \emph{ratios} to it rather than independent magnitudes; moreover the
geometric span of a single plan is the horizon reach $L=v\,T\,\Delta t$, making
$v$ and the horizon $T$ partially redundant. These refinements do not change the
analysis, which requires only that $\widetilde{\mathbf h}^{\mathcal M}_t$ be a
bounded, continuous function of the augmented state, held fixed during the MPPI
update.

Crucially, $\mathcal M_t$ is a buffer of fixed size $P$: the controller carries
a \emph{constant-memory fading history}, of size independent of the run length,
rather than the growing trajectory history of classical spectral methods, and
it is not a memoryless map. Its presence augments the closed-loop Markov state,
which we make explicit in Sec.~\ref{sec:guarantees}.
}
...
The \cl{fading-memory-augmented} curl-augmented Stein field
\cl{$\widetilde{\mathbf h}^{\mathcal M}_t(\mathbf z)$ of Sec.~\ref{sec:memory_flow}} provides \cl{a desired spatial transport direction at every point of the workspace}.
We now incorporate this field into MPPI as an additive rollout cost. The goal is not to explicitly perform gradient descent on the control sequence, but to assign lower cost to sampled trajectories whose spatial \cl{motion} is aligned with the reference flow. \cl{To keep the notation of this subsection light we write $\widetilde{\mathbf h}_t$ for the tracked field, understood to be $\widetilde{\mathbf h}^{\mathcal M}_t$ whenever the memory feedback is active.}
...
-
\widetilde{\mathbf h}_t(\mathbf z_{t,k}^{(b)})
\right\|^2 ,}
\end{equation}
...
-\frac{2}{\Delta t}\,\widetilde{\mathbf h}_t(\mathbf z_{t,k}^{(b)})^{\top}\Delta\mathbf z_{t,k}^{(b)}
+\big\|\widetilde{\mathbf h}_t(\mathbf z_{t,k}^{(b)})\big\|^2
\right].}
\end{equation}
...
\sum_{k=0}^{T-1}
\left[
-\,\Delta t\;\widetilde{\mathbf h}_t(\mathbf z_{t,k}^{(b)})^{\top}\Delta\mathbf z_{t,k}^{(b)}
...
\cl{This is minimized precisely when the rollout's spatial increments follow the reference flow, $\Delta\mathbf z_{t,k}^{(b)}\approx\Delta t\,\widetilde{\mathbf h}_t(\mathbf z_{t,k}^{(b)})$, i.e.\ $\dot{\mathbf z}\approx\widetilde{\mathbf h}_t(\mathbf z)$. Unlike a position-alignment reward, it is therefore a genuine first-order flow-tracking objective rather than a reward on absolute position, and it is invariant to a constant spatial shift of the workspace. The reference field $\widetilde{\mathbf h}_t$ is evaluated once from the current rollout ensemble \cl{and fading memory} and held fixed during the MPPI update, so the cost is evaluated directly on sampled rollouts without differentiating through the dynamics or through the reference field.}
...
when the surrogate drives the importance weights in practice\cl{, and for the fading-memory-augmented field $\widetilde{\mathbf h}^{\mathcal M}_t$, which enters only through the reference value $\widetilde{\mathbf h}_t(\mathbf z_t)$}.}
...
\begin{aligned}
    ...
    -
    \widetilde{\mathbf h}_t(\mathbf z_t)
    \right\|^2}
    &\leq
    \cl{2\left\|
    \mathbf v_t^{\mathrm{exec}}
