...
from experiments.literature_config import GMMSpec, LiteratureComparisonConfig
from experiments.scenarios import Scenario
from models import double_integrator as model
from mppi.core import ObstacleParams
from mppi.stein import pdf
from scripts.main import _random_state, closed_loop_jit, multi_robot_closed_loop_jit
...
def make_no_obstacle_scenario(
    ...
    *,
    ...
    stein = dataclasses.replace(
        base_scenario.params.stein,
        means=means,
        cov_inv=cov_inv,
    ...
    obstacle_params = ObstacleParams(
        xyr=jnp.zeros((0, 3), dtype=jnp.float32),
        weight=0.0,
        safe_distance=float(base_scenario.params.obstacle_params.safe_distance),
    )
    ...
    return Scenario(
        ...
        run_config=params_loader.RunConfig(
            seed=int(base_scenario.run_config.seed),
            steps=int(steps),
            num_robots=int(team_size),
        ),
...
def _clamp_controls_np(u: np.ndarray, model_params) -> np.ndarray:
    ...
    out[:, 0] = np.clip(out[:, 0], -model_params.max_accel_lin_abs, model_params.max_accel_lin_abs)
    out[:, 1] = np.clip(out[:, 1], -model_params.max_accel_lin_abs, model_params.max_accel_lin_abs)
    out[:, 2] = np.clip(out[:, 2], -model_params.max_accel_ang_abs, model_params.max_accel_ang_abs)
    return out
...
def _double_integrator_step_np(x: np.ndarray, u: np.ndarray, model_params) -> np.ndarray:
    ...
    new_px = px + vx * dt + 0.5 * ax * dt * dt
    new_py = py + vy * dt + 0.5 * ay * dt * dt
    new_vx = vx + ax * dt
    new_vy = vy + ay * dt
    new_yaw = yaw + yaw_rate * dt + 0.5 * w * dt * dt
    new_yaw_rate = yaw_rate + w * dt

    return np.stack([new_px, new_py, new_vx, new_vy, new_yaw, new_yaw_rate], axis=1)
...
def _tracker_step_np(
    ...
    controls = _clamp_controls_np(controls, scenario.params.model_params)
    next_states = _double_integrator_step_np(states, controls, scenario.params.model_params)
...
def _run_mppi(
    ...
    *,
    ...
    if team_size == 1:
        ...
        path, _, _ = closed_loop_jit(params, x0_j[0], U0, key, N=steps)
        return np.asarray(path, dtype=np.float64)[:, None, :]
    ...
    return np.asarray(paths_all, dtype=np.float64)
...
def _rollout_controls(x0: jnp.ndarray, controls: jnp.ndarray, model_params) -> jnp.ndarray:
    def one_step(x, u_t):
        x_next = jax.vmap(model.step, in_axes=(0, 0, None))(x, u_t, model_params)
...
def _run_traj_opt(
    ...
    *,
    ...
    model_params = scenario.params.model_params
