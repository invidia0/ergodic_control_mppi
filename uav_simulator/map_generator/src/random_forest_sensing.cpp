...
void RandomMapGenerateCylinder() {
  ...
  for (int i = 0; i < _obs_num && rclcpp::ok(); i++) {
    ...
    // Qualify spacing on the centers that are actually published, after grid snapping.
    x = floor(x / _resolution) * _resolution + _resolution / 2.0;
    y = floor(y / _resolution) * _resolution + _resolution / 2.0;
    ...
    // ObstacleShape/lower_rad and upper_rad are radii. Upstream treated them as a
    // diameter and multiplied by another random scale, so a requested 0.3--0.6 m
    // pillar silently became 0.075--0.45 m.
    int widNum = ceil((2.0 * radius) / _resolution);
    bool first_cell = true;

    for (int r = -widNum / 2.0; r < widNum / 2.0; r++)
      for (int s = -widNum / 2.0; s < widNum / 2.0; s++) {
        // Preserve the upstream draw count so this height fix does not change seeded XY maps.
        const double sampled_height = rand_h(eng);
        if (first_cell) {
          h = sampled_height;
          first_cell = false;
        }
...
int main(int argc, char **argv)
{
    ...
    // Transient-local, matching mockamap. The map is static, so a consumer that starts
    // after this node must still receive it; with the default volatile durability a
    // transient-local subscriber does not even match this publisher and gets nothing.
    ...
    // Seed the generator. Upstream always drew from std::random_device, which makes the
    // map different on every launch -- fine for a planner demo, useless for an experiment
    // that has to be reproducible or compared across arms. A negative `seed` keeps the old
    // nondeterministic behaviour; any non-negative value pins the map.
    int requested_seed = 0;
    node->declare_parameter("seed", -1);
    node->get_parameter("seed", requested_seed);
    unsigned int seed = requested_seed < 0 ? rd() : static_cast<unsigned int>(requested_seed);
    RCLCPP_INFO(node->get_logger(), "map seed=%u (%s)", seed,
                requested_seed < 0 ? "random" : "fixed");
    eng.seed(seed);
