...
private:
    ...
    rclcpp::Time last_position_cmd_time_;
...
void SO3ControlComponent::position_cmd_callback(const quadrotor_msgs::msg::PositionCommand::ConstPtr &cmd)
{
    ...
    last_position_cmd_time_ = this->get_clock()->now();
...
{   
    ...
    if (position_cmd_init_)
    {
        ...
        //
        // Watchdog: after command_timeout_ without a setpoint, fall back to holding the
        // current position. Without this the last setpoint keeps its velocity and
        // acceleration feedforward applied indefinitely, so losing the command source
        // drives the vehicle away rather than stopping it.
        if ((this->get_clock()->now() - last_position_cmd_time_).seconds() > command_timeout_)
        {
            des_pos_ = position;
            ...
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                                 "position_cmd timeout, holding current position");
        }
...
void SO3ControlComponent::onInit(void)
{
    ...
    declare_parameter("command_timeout", 0.10);
