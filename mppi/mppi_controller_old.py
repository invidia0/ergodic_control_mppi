...
from mppi.dynamics import Bicycle, SingleIntegrator, DoubleIntegrator
...
class MPPIController():
    def __init__(
        ...
        with jax.default_device(self.device):
            ...
            num_exploration = jnp.asarray((1.0 - self.param_exploration) * self.K, dtype=jnp.int32)
            self.use_nominal = jnp.arange(self.K) < num_exploration
            self.param_lambda = param_lambda
            self.param_alpha = param_alpha
            self.param_gamma = self.param_lambda * (1.0 - (self.param_alpha))
            self.Sigma = sigma
            self.Sigma_chol = jnp.linalg.cholesky(self.Sigma + 1e-8 * jnp.eye(self.dim_u))
            self.Sigma_inv = jnp.linalg.inv(self.Sigma + 1e-8 * jnp.eye(self.dim_u))
    ...
    def _mppi_step(self, x0, U_prev, key):
        # 1) sample epsilon
        ...
        # 2) rollouts
        S_V = jax.vmap(self.single_rollout, in_axes=(None, None, 0), out_axes=(0, 0, 0))
        S, V, trajs = S_V(x0, U_prev, epsilon) # S: (K,); V: (K, T, dim_u); trajs: (K, T, dim_x)
    ...
    def _c(self, x_t: jnp.ndarray, u_t: jnp.ndarray) -> float:
        """
