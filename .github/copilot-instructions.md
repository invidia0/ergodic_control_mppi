...
- `mppi/mppi_core.py` holds the functional MPPI implementation and trajectory rollout logic. Keep code here side-effect free and JAX-friendly.
...
- Avoid adding side effects inside `jax.jit` or `jax.lax.scan` paths in `mppi_core.py` and sweep helpers.
