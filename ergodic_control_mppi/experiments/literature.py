"""Run and summarize the configured literature-method comparison."""
...
class GMMSpec:
    """Named Gaussian-mixture scenario arrays."""
    name: str
...
@dataclass(frozen=True)
class HedacConfig:
    """HEDAC baseline discretization and solver parameters."""
    grid_size: int
...
@dataclass(frozen=True)
class TrajOptConfig:
    """Trajectory-optimization baseline parameters."""
    iterations: int
    learning_rate: float
...
class LiteratureComparisonConfig:
    """Validated literature-comparison experiment configuration."""
    scenario_name: str
...
from ergodic_control_mppi.experiments.common import append_csv, load_scenario, prepare_outputs, summarize
...
from ergodic_control_mppi.metrics.ergodicity import compute_cumulative_team_ergodic_error
...
def run_literature_comparison(
    config_path: str = "configs/experiments/literature_comparison.yaml",
    ...
    prepare_outputs((out_csv, summary_csv, convergence_csv), overwrite)
    ...
    for scenario in scenarios:
        for seed in cfg.seeds:
            ...
            for method_name in cfg.methods:
                ...
                row.update(metrics)
                all_rows.append(row)
                append_csv(out_csv, row, row_fields)
                ...
                for t_idx, val in enumerate(np.asarray(cumulative, dtype=np.float64), start=1):
                    ...
                    append_csv(convergence_csv, conv_row, conv_fields)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/experiments/literature_comparison.yaml",
        help="Path to literature comparison YAML config.",
    )
    parser.add_argument("--overwrite", action="store_true")
    args = parser.parse_args()
    run_literature_comparison(args.config, overwrite=args.overwrite)
